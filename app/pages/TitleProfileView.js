        ...
        <div className="content cont-titles" id="content">
          ...
          <div className="row titles">
            {profileData.titles.map((title) => {
              return <div key={title.id} className="title col-md-4 col-6"><a
                href={`https://wowhead.com/?title=${title.id}`} target="_blank" rel="noopener noreferrer"
                className="title-rect">{title.name}</a></div>
            })};
          </div>
        </div>
      </div>
